...
namespace Расписание
{
    ...
    {
        public static List<TimingTable> listTiming = DBHelper.getListTimingTable();
        static List<Worker> listWorker = DBHelper.getWorkerList();

        public static void makeSchedule(int countDays, int hourStart, DateTime dayStart, string timeTable)
        {
            int days = Convert.ToInt32(timeTable.Split('/')[0]); //количество рабочих дней
            if (listTiming.Count == 0)
            {
                int id = 1;                
